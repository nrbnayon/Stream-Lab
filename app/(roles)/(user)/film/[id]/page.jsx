...
export default function FilmDetails() {
  ...
  // If not in library (404 error), fetch general film details
  const shouldFetchGeneralDetails = libraryError?.status === 404;
  ...
  const {
    ...
    skip: !shouldFetchGeneralDetails,
  });

  // Determine which data to use
  const isLoading =
    libraryLoading || (shouldFetchGeneralDetails && generalLoading);
  const error = shouldFetchGeneralDetails ? generalError : libraryError;
  const filmResponse = libraryResponse || generalResponse;
  ...
  const videoSrc =
    hasFullAccess && film_hls_url ? film_hls_url : trailer_hls_url;
  ...
  return (
    <div className="grid gap-5">
      ...
      <section className="my-5 space-y-5">
        ...
        {!hasLibraryAccess && trailer_hls_url && (
